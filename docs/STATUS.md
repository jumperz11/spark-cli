...
| `spark config get|set|unset|list` with dotted keys + JSON coercion (`config get` prints `null` when a key is explicitly set to JSON null and rejects malformed dotted keys) | `cmd_config_*`, `dotted_get`, `dotted_set`, `dotted_unset`, `coerce_config_value` | `test_dotted_*`, `test_coerce_config_value_*`, `test_config_get_*` |
